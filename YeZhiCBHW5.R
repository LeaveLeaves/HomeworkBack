...
get_contrasts_in_subtree = function(node, tree, description) {
    ...
    # for the child nodes, saving the updated description.  
      ...
      u1 = get_contrasts_in_subtree(child1, tree, description)
      u2 = get_contrasts_in_subtree(child2, tree, u1)
      C1 = calculate_corrected_branch_length(node, child1, child2, u2$corrected_branch_lengths)
      C21 = calculate_trait_and_contrast(child1, child2, u2$traits[,1], u2$corrected_branch_lengths)
      u2$traits[node,1] = C21$trait_value
      u2$normalized_contrasts[node,1] = C21$contrast
  
    # Update the branch length leading to the given node, save it in the description
      u2$corrected_branch_lengths[node] = C1

    # Compute the trait values and normalized contrasts for both traits, save the values in the description
      C22 = calculate_trait_and_contrast(child1, child2, u2$traits[,2], u2$corrected_branch_lengths)
      u2$traits[node,2] = C22$trait_value
      u2$normalized_contrasts[node,2] = C22$contrast
...
get_trait_correlation = function(newick_tree, traits_at_tip) {	
    ...
    node = get_root(tree)
    updated_description = get_contrasts_in_subtree(node, tree, description) 

    # Perform linear regression on the traits at the tips and get the slope, intercept, P-value, and R2-value
    raw_data_regression = linear_regression(description$traits[,1], description$traits[,2])
